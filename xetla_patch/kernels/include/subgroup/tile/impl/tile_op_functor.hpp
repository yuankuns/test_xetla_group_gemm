...
struct dequant_int4_weight_t {
  ...
  __XETLA_API KERNEL_FUNC void operator()(
    ...
    // no tail, because this is matB
      ...
      //         "args (m %d n %d k %d)\n", args.wg_start_m, args.wg_start_n, args.wg_start_k);
      // }
...
#pragma unroll
      for (uint32_t j = 0; j < num_block_x; ++j) {
        ...
        //     block_id * matB_acc_t::block_elems + matB_acc_t::block_elems / 2);

    // if (debug) {
    //     auto matB_r=matB.reg.xetla_format<uint8_t>().
    //         xetla_select<matB_acc_t::block_elems / 2, 1>(
    //             block_id * matB_acc_t::block_elems / 2);
    //     for (int i = 0; i < matB_acc_t::block_elems / 2; ++i) {
    //         matB_acc.reg[i] = float(sycl::ext::oneapi::bfloat16(matB_r.xetla_select<1, 1>(i)));
    //     }
    //     // auto matB_r=matB.reg.xetla_format<uint8_t>();
    //     // for (int i = 0; i < 64; ++i) {
    //     //     matB_acc.reg[i] = float(sycl::ext::oneapi::bfloat16(matB.reg.xetla_select<1, 1>(i)));
    //     // }
    // }
    // return;
...
#pragma unroll
          for (uint32_t ii = 0; ii < block_size_y_b; ii += step) {
            ...
            uint32_t scale_idx =
                ...
                offset_x_in_tile; // ?
            // if (debug) {
            //     matB_acc.reg[jj * block_size_y_b + ii] = float(scale_idx);
            // }
            // if ((jj == block_size_x_b - 1) and (ii == block_size_y_b-1))
            //     return;
            ...
            dst_blk.xetla_select<step, 1>(jj * block_size_y_b + ii) =
                cvt_blk_i8.xetla_select<step, 1>(jj * block_size_y_b + ii) *
                scale.reg[scale_idx];
            // dst_blk.xetla_select<step, 1>(jj * block_size_y_b + ii) = float(sycl::ext::oneapi::bfloat16(scale.reg.xetla_select<1, 1>(scale_idx)));
            // dst_blk.xetla_select<step, 1>(jj * block_size_y_b + ii) = cvt_blk_i8.xetla_select<step, 1>(jj * block_size_y_b + ii);
                // cvt_blk_i8.xetla_select<step, 1>(jj * block_size_y_b + ii);
             //    float(sycl::ext::oneapi::bfloat16(scale.reg.xetla_select<1, 1>(scale_idx)));
             // cvt_blk_i8.xetla_select<step, 1>(jj * block_size_y_b + ii) *
                // scale.reg[scale_idx];
            ...
            // sycl::ext::oneapi::experimental::printf("int4 debug: scale[%d] %f\n", scale_idx, float(sycl::ext::oneapi::bfloat16(scale.reg.xetla_select<1, 1>(scale_idx))));
            // sycl::ext::oneapi::experimental::printf(
            //     "scale[%d] %f \n",
            //     scale_idx,
