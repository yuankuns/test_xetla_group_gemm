...
cgfs_t moe_gemm_int4(
    ...
    const int* total_rows_for_experts_host,
    const int problem_count) {
  // use the same tiling policy with FP8
  using Policy = gpu::xetla::MoEGEMMFP8Policy;
#define LAUNCH_INT4_MOE_IMPL(GROUP_SIZE)                         \
  do {                                                           \
    return gpu::xetla::LaunchMoEGEMMINT4<T, Policy, GROUP_SIZE>( \
        queue,                                                   \
        activations,                                             \
        weights,                                                 \
        scales,                                                  \
  ...
  switch (group_size) {
    case 32:
      LAUNCH_INT4_MOE_IMPL(32);
      break;
    case 64:
      LAUNCH_INT4_MOE_IMPL(64);
      break;
    case 128:
      LAUNCH_INT4_MOE_IMPL(128);
      break;
    case 256:
      LAUNCH_INT4_MOE_IMPL(256);
      break;
    default:
      TORCH_CHECK(
          false,
          "Error in moe_gemm_int4: Unsupported group_size, only support 32, 64, 128, 256 now.");
  }
#undef LAUNCH_INT4_MOE_IMPL
}

// generate the template instantiation for sycl::half and
// sycl::ext::oneapi::bfloat16
template XETLA_KERNEL_API cgfs_t moe_gemm_int4<sycl::half>(
    sycl::queue& queue,
    const sycl::half* activations,
    const uint32_t* weights,
...
template XETLA_KERNEL_API cgfs_t moe_gemm_int4<sycl::ext::oneapi::bfloat16>(
    sycl::queue& queue,
    const sycl::ext::oneapi::bfloat16* activations,
    const uint32_t* weights,
    const sycl::ext::oneapi::bfloat16* scales,
    sycl::ext::oneapi::bfloat16* outputs,
    const int total_m,
    const int gemm_n,
    const int gemm_k,
